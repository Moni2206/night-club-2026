    ...
    <motion.div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <Link href={href} className={`relative px-4 py-2 block cursor-pointer ${active ? "text-[var(--pink)]" : "text-[var(--headlines)]"}`}>
        <div className="relative overflow-hidden h-6">
          <div className="flex">
            {label.split("").map((l, i) => (
              <motion.span key={i} initial={{ y: 0 }} animate={shouldAnimate ? { y: "-100%" } : { y: 0 }} transition={{ duration: DURATION, delay: STAGGER * i, ease: "easeInOut" }}>
                {l}
              </motion.span>
            ))}
          </div>
          {!active && (
            <div className="absolute inset-0 flex text-[var(--pink)]">
              {label.split("").map((l, i) => (
                <motion.span key={i} initial={{ y: "100%" }} animate={shouldAnimate ? { y: 0 } : { y: "100%" }} transition={{ duration: DURATION, delay: STAGGER * i, ease: "easeInOut" }}>
                  {l}
                </motion.span>
              ))}
            </div>
